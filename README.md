...
7. In AWS Console, go to `Settings` menu. There, click on `Edit` and turn-on `Stream configuration changes and notifications to an Amazon SNS topic.`. Then select a SNS topic we created in the step 3, called `EncryptionDisabled` <details>![Config Settings](/step7.png)</details>   
